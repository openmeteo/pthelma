...
B<loggertodb> reads a plain text data file, connects to the hydria database,
...
written by Antonis Christofides, for the openmeteo.org database.
B<loggertodb> is essentially B<autoupdate> adapted to the hydria
database for the Odysseus project.  This version of B<loggertodb> has
nothing to do with versions prior to 1.0.0, which were completely
different, in a different programming language (Perl rather than
Python), and not based on B<autoupdate>.
